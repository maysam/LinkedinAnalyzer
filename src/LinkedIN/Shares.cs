...
namespace LinkedIN
{
	...
	public partial class LinkedINRestClient
	{
		...
		public Shares RetrieveCurrentMemberUpdates( IEnumerable< ProfileField > fields, int start = 0, int count = 500 )
		{
			// validate arguments
			if ( fields == null )
			...
			return RetrieveUpdates( "~", fields, start, count );
		}
		/// <summary>
		...
		/// <exception cref="LinkedINHttpResponseException">Thrown when the an unexepcted response was returned from LinkedIN.</exception>
		/// <exception cref="LinkedINUnauthorizedException">Thrown when an request was made to an protected resource without the proper authorization.</exception>
        public Shares RetrieveMemberUpdates(string id, IEnumerable<ProfileField> fields, int start = 0, int count = 500)
		{
			// validate arguments
			...
			// retrieve the profile
			return RetrieveUpdates( "idPart=" + id, fields, start, count );
		}
		/// <summary>
		...
		/// <exception cref="LinkedINHttpResponseException">Thrown when the an unexepcted response was returned from LinkedIN.</exception>
		/// <exception cref="LinkedINUnauthorizedException">Thrown when an request was made to an protected resource without the proper authorization.</exception>
		protected Shares RetrieveUpdates( string idPart, IEnumerable< ProfileField > fields, int start, int count )
		{
			// validate arguments
			...
			// create the request
			var request = new RestRequest( "people/" + idPart + "/shares" + FieldSelector.ToFieldSelector( fields ) );
			...
			// execute the request
			return ExecuteRequest< Shares >( request );
		}
		#endregion
	}
}